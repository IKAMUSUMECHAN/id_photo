...
def cut_photo(photo,choice):
    ...
    if choice == 3:
        if rate < (HEIGHT_3IN / WIDTH_3IN):
            x = (width - int(height / HEIGHT_3IN * WIDTH_3IN)) / 2
            y = 0
            cutted_photo = photo.crop((x, y, x + (int(height / HEIGHT_3IN * WIDTH_3IN)), y + height))

        else:
            x = 0
            y = (height - int(width / WIDTH_3IN * HEIGHT_3IN)) / 2
            cutted_photo = photo.crop((x, y, x + width, y + (int(width / WIDTH_3IN * HEIGHT_3IN))))
        return cutted_photo
...
def resize_photo(photo,choice):
    ...
    if choice == 2:
        ...
        return resized_photo
    if choice == 3:
        resized_photo = photo.resize((WIDTH_3IN, HEIGHT_3IN))
        return resized_photo
    if choice == 0:
        return photo
...
def layout_photo_6_1(photo):
    ...
    # 创建画笔
    draw = ImageDraw.Draw(bk)
    #draw.line([(0,WIDTH_6IN*0.25),(WIDTH_6IN*0.5,WIDTH_6IN*0.25)],fill=128) # 1横线
    #draw.line([(0,WIDTH_6IN*0.5),(WIDTH_6IN*0.5,WIDTH_6IN*0.5)],fill=128) # 2横线
    #draw.line([(0,WIDTH_6IN*0.75),(WIDTH_6IN,WIDTH_6IN*0.75)],fill=128) # 3横线
    #draw.line([(HEIGHT_6IN*0.25,0),(HEIGHT_6IN*0.25,WIDTH_6IN*0.5)],fill=128) # 竖线
    #draw.line([(HEIGHT_6IN*0.5,0),(HEIGHT_6IN*0.5,WIDTH_6IN*0.5)],fill=128) # 竖线
    #draw.line([(HEIGHT_6IN*0.75,0),(HEIGHT_6IN*0.75,WIDTH_6IN*0.5)],fill=128) # 竖线
...
def layout_photo_6_2(photo):
    ...
    :return: 处理后的照片
    """
    bk = Image.new("RGB", [HEIGHT_6IN,WIDTH_6IN], (255,255,255))
    # 创建画笔
    #draw = ImageDraw.Draw(bk)
    #draw.line([(0, HEIGHT_6IN*0.54),(WIDTH_6IN*0.5, HEIGHT_6IN*0.54)],fill=128) # 1横线
    #draw.line([(0, HEIGHT_6IN * 1.09), (WIDTH_6IN * 0.5, HEIGHT_6IN * 1.09)], fill=128)  # 2横线
    ...
    focus_point = [0.125 * WIDTH_6IN, 0.27 * HEIGHT_6IN]
    start_point = [focus_point[0] - 0.5 * WIDTH_2IN, focus_point[1] - 0.5 * HEIGHT_2IN]
    for i in range(0, 2):
        for k in range(0, 2):
            bk.paste(photo, (int(start_point[0] + (k * WIDTH_6IN *0.25)), int(start_point[1] + 0.54 * i * HEIGHT_6IN)))
    #bk.show()
    return bk

def layout_photo_6_3(photo):
    """
    在6寸照片上排版3寸照片
    :param photo: 待处理照片3寸
    :return: 处理后的照片
    ...
    # 创建画笔
    draw = ImageDraw.Draw(bk)
    #draw.line([(0, HEIGHT_6IN/1.2), (WIDTH_6IN*0.8, HEIGHT_6IN/1.2)], fill=128)  # 横线
    #draw.line([(WIDTH_6IN*0.39, 0), (WIDTH_6IN*0.39, HEIGHT_6IN*0.833)], fill=128)  # 第1条竖线
    #draw.line([(WIDTH_6IN*0.8, 0), (WIDTH_6IN*0.8, HEIGHT_6IN*0.833)], fill=128)  # 第2条竖线
    #draw.line([(WIDTH_6IN*0.75,0),(WIDTH_6IN*0.75,HEIGHT_6IN)],fill=128) # 第3条竖线
    focus_point = [0.125 * WIDTH_6IN, 0.25 * HEIGHT_6IN]
    start_point = [focus_point[0] - 0.35 * HEIGHT_2IN, focus_point[1] - 0.7 * WIDTH_2IN]
    for i in range(0, 2):
        for k in range(0, 1):
            bk.paste(photo, (int(start_point[0] + (k * WIDTH_6IN / 2.5)), int(start_point[1] + 0.57 * i * HEIGHT_6IN)))
    #bk.show()
    return bk
...
def layout_photo_6_mix1(photo1, photo2):
    ...
    # 创建画笔
    draw = ImageDraw.Draw(bk)
    # draw.line([(0,HEIGHT_6IN*0.5),(WIDTH_6IN*0.5, HEIGHT_6IN/2)],fill=128)  # 横线
    ...
    # draw.line([(WIDTH_6IN*0.25, 0), (WIDTH_6IN*0.25, HEIGHT_6IN*0.75)],fill=128)  # 第1条竖线
    # draw.line([(WIDTH_6IN*0.5,0),(WIDTH_6IN*0.5,HEIGHT_6IN)],fill=128)  # 第2条竖线
    # draw.line([(WIDTH_6IN*0.75,0),(WIDTH_6IN*0.75,HEIGHT_6IN)],fill=128)  # 第3条竖线
    ...
    start_point2 = [focus_point2[0] - 2.68 * WIDTH_2IN, focus_point2[1] + 0.5 * HEIGHT_2IN]
    for i in range(0, 1):
        for k in range(0, 2):
...
# layout_photo_6_3(resize_photo(cut_photo(im, 3), 3).rotate(90, expand=True)).save('6_3.jpg')
...
#layout_photo_6_mix2(resize_photo(cut_photo(im,1),1),resize_photo(cut_photo(im,2),2)).save('6_mix2.jpg')
    resize_photo(cut_photo(im,1),1).save("done1.jpg")